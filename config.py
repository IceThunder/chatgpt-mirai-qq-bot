...
class GeminiAIParams(BaseModel):
...
class GeminiAuths(BaseModel):
    api_endpoint: Optional[str] = None
    """自定义 Gemini API 的接入点"""
    temperature: float = 0.7
    top_p: float = 1.0

    gemini_params: GeminiAIParams = GeminiAIParams()

    accounts: List[GeminiAIAPIKey] = []
    """Gemini的账号列表"""
...
class Config(BaseModel):
    ...
    gemini: GeminiAuths = GeminiAuths()
